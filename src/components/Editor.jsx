...
export default function Editor({ title, setTitle, description, setDescription, cancelStory, saveStory, deleteStory }) {
  ...
  function createStory(e) {
    ...
    const prompt = `Escreva uma história com um(a) protagonista "${hero}" com um conflito do tipo "${conflict}" do gênero "${genre}" ambientada em um(a) "${ambient}" no tempo "${time}" para o público "${audience}". ${additional}. Lembre-se de incluir um título no início do texto.`;
  ...
  return (
    <div className="editor">
      ...
      <button onClick={deleteStory} id="btnExcluir" style={{visibility:'hidden'}}>Excluir</button>
    </div>
  );
}
